...
const ProjectApprovalManager = () => {
  ...
  const [form, setForm] = useState({ 'role_id': '5' });
  const [formError, setFormError] = useState({});
  const [searchTerm, setSearchTerm] = useState('');
  const [projectAppManagers, setProjectAppManagers] = useState([]);
  ...
  // * project managers fecth function
  const getProjectAppManagers = async () => {
    setLoading(true);
    try {
      const res = await authClient.get('users/5');
      if (res?.data?.status) {
        setProjectAppManagers(res?.data?.data);
      }
    } catch (e) {
      console.log(e)
    } finally {
  ...
  const handleSubmit = async () => {
    if (validate()) { 
      ...
      let res;
      try {
        res = await authClient.post('createUser', form);
        if (res?.data?.status) {
          ...
          toastError('Error in saving Project Approval Manager')
        }
      } catch (err) {
        console.log(err);
        toastError('Error in saving Project Approval Manager')
          ...
          <div className="container-fluid">
                ...
                <div className="page-title-box d-flex align-items-center justify-content-between">
                  <div className="page-title-right">
                    <ol className="breadcrumb m-0">
                      ...
                      <li className="breadcrumb-item active"> Project Approval Manager</li>
                    </ol>
                  </div>
                </div>
              </div>
              ...
              <div className='col-md-6'>
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">Project Approval Manager </h3>
                  </div>
                        ...
                        <div className="col-6">
                          <div className="form-floating">
                            <input
                              ...
                              onChange={handleChange}
                            />
                            <label htmlFor="contact" className="fw-normal">Contact</label>
            ...
            <div className="row justify-content-center">
              <div className="col-md-12">
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">Project Approval Managers</h3>
                    <div className="">
                      <input
                        type="text"
                        className="form-control"
                  ...
                  <div className="card-body">
                    <div className="table-responsive-md">
                      <table className="table text-nowrap mb-0">
                        ...
                        <tbody>
                          {projectAppManagers.length > 0 ? (
                            projectAppManagers.map((project, index) => (
                              <tr key={index}>
                                <td>{index + 1}</td>
                                <td>{AllCities.find((a) => a.city_code == project?.city_code)?.city_name}</td>
                            ...
                            <tr>
                              <td colSpan="5">No project managers available.</td>
                            </tr>
                          )}
                        </tbody>
                      </table>
