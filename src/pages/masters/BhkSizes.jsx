...
const BhkSizes = () => {
  ...
  const [bhkSizes, setBhkSizes] = useState([]);
  ...
  const validate = () => {
    ...
    if (!form.name) {
      isFormValid = false;
      errors.name = 'Please Enter BHK Size';
    }
    if (!form.description) {
      isFormValid = false;
  ...
  const handleSubmit = async () => {
    if (validate()) {
      let res;
      if (update?.id) {
        res = await masterClient.put(`bhksizes/${update.id}`, form);
      } else {
        res = await masterClient.post('bhksizes', form);
      }
      try {
        setLoading(true);
        if (res?.data?.status) {
  ...
  // delete BHK Sizes
  const deleteBhkSize = async (propId) => {
    setLoading(true);
    try {
      const res = await masterClient.delete(`bhksizes/${propId}`);
      if (res?.data?.status) {
        toastSuccess(res?.data?.message);
        getBhkSizes();
      }
          ...
          <div className="container-fluid">
                ...
                <div className="page-title-box d-flex align-items-center justify-content-between">
                  <div className="page-title-right">
                    <ol className="breadcrumb m-0">
                      ...
                      <li className="breadcrumb-item active">Add BHK Sizes</li>
                    </ol>
                  </div>
                  <div className="page-title-right">
              ...
              <div className="col-md-12">
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">BHK Sizes</h3>
                    <div className="">
                      <input
                        type="text"
                            ...
                            <tr key={index}>
                              ...
                              <td className="table-icons">
                                <tr>
                                  ...
                                  <td onClick={() => deleteBhkSize(item.id)}>
            ...
            <Offcanvas show={show} onHide={() => setShow(false)} placement="end">
              ...
              <Offcanvas.Body>
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">Add BHK Size</h3>
                  </div>
                  <div className="card-body">
