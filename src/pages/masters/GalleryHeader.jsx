...
const GalleryHeader = () => {
  ...
  const validate = () => {
    ...
    if (!form.name) {
      isFormValid = false;
      errors.name = 'Please Enter Gallery Header';
    }
    if (!form.description) {
      isFormValid = false;
  ...
  const handleSubmit = async () => {
    if (validate()) {
      let res;
      if (update?.id) {
        res = await masterClient.put(`galleryheaders/${update.id}`, form);
      } else {
        res = await masterClient.post('galleryheaders', form);
      }
      try {
        setLoading(true);
        if (res?.data?.status) {
  ...
  const deletgalleryHeader = async (feId) => {
    setLoading(true);
    try {
      const res = await masterClient.delete(`galleryheaders/${feId}`);
      if (res?.data?.status) {
        toastSuccess(res?.data?.message);
        getgalleryHeaders();
      }
