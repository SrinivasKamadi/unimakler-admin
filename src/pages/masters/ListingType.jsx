...
const ListingType = () => {
  ...
  const [form, setForm] = useState({ order: 1 });
  ...
  const validate = () => {
    ...
    if (!form.name) {
      isFormValid = false;
      errors.name = 'Please Enter Listing Type';
    }
    if (!form.description) {
      isFormValid = false;
  ...
  const handleSubmit = async () => {
    if (validate()) {
      let res;
      if (update?.id) {
        res = await masterClient.put(`listingtype/${update.id}`, form)
      }
      else {
        res = await masterClient.post('listingtype', form);
      }
      try {
        setLoading(true);
        if (res?.data?.status) {
  ...
  // delete BHK Sizes
  const deleteListingType = async (propId) => {
    setLoading(true);
    try {
      const res = await masterClient.delete(`listingtype/${propId}`);
      if (res?.data?.status) {
        toastSuccess(res?.data?.message);
        getListingTypes();
      }
  ...
  // edit Villa Types
  const handleEdit = (ListingData) => {
    setShow(true);
    setForm({ ...ListingData });
    setUpdate(ListingData);
  }
  useEffect(() => {
    getListingTypes();
  }, []);
          ...
          <div className="container-fluid">
                ...
                <div className="page-title-box d-flex align-items-center justify-content-between">
                  <div className="page-title-right">
                    <ol className="breadcrumb m-0">
                      ...
                      <li className="breadcrumb-item active">Add Listing Type</li>
                    </ol>
                  </div>
                  <div className="page-title-right">
              ...
              <div className="col-md-12">
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">Listing Types</h3>
                    <div className="">
                      <input
                        type="text"
            ...
            <Offcanvas show={show} onHide={() => setShow(false)} placement="end">
              ...
              <Offcanvas.Body>
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">Add Listing Type</h3>
                  </div>
                  <div className="card-body">
