...
export default function SalebleAreaRepresentation() {
  ...
  const validate = () => {
    ...
    if (!form.name) {
      isFormValid = false;
      errors.name = 'Please Enter Saleable Area';
    }
    if (!form.description) {
      isFormValid = false;
  ...
  const handleSubmit = async () => {
    if (validate()) {
      let res;
      if (update?.id) {
        res = await masterClient.put(`saleable-area-representation/${update.id}`, form)
      }
      else {
        res = await masterClient.post('saleable-area-representation', form);
      }
      try {
        setLoading(true);
        if (res?.data?.status) {
  ...
  // get  BHK Sizes
  const getSaleableAreaRep = async () => {
    setLoading(true);
    try {
  ...
  // delete BHK Sizes
  const deleteArea = async (propId) => {
    setLoading(true);
    try {
      const res = await masterClient.delete(`saleable-area-representation/${propId}`);
      if (res?.data?.status) {
        toastSuccess(res?.data?.message);
        getSaleableAreaRep();
      }
          ...
          <div className="container-fluid">
                ...
                <div className="page-title-box d-flex align-items-center justify-content-between">
                  <div className="page-title-right">
                    <ol className="breadcrumb m-0">
                      ...
                      <li className="breadcrumb-item active">Add Saleable Area Representation</li>
                    </ol>
                  </div>
                  <div className="page-title-right">
                    <button onClick={() => setShow(true)} className="btn btn-info">
                      Add Saleable Area Representation
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="col-md-12">
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">Saleable Area Representations</h3>
                    <div className="">
                      <input
                        type="text"
            ...
            <Offcanvas show={show} onHide={() => setShow(false)} placement="end">
              ...
              <Offcanvas.Body>
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">Add Saleable Area Representation</h3>
                  </div>
                  <div className="card-body">
