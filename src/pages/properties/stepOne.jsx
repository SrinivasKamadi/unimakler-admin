...
const stepOne = ({
  ...
  const getPropertyTypes = async () => {
    ...
    try {
      ...
      console.log('get property types========', res);
      if (res?.data?.status) {
  ...
  const handleChange = async (e) => {
    ...
    setForm((prev) => {
      ...
      if (name === 'sale') {
        ...
        newState.rent = false; // Uncheck rent if sale is selected
      } else if (name === 'rent') {
        newState.rent = !prev.rent;
        newState.sale = false; // Uncheck sale if rent is selected
      } else {
